...
class Mat:
    ...
    def mul_then_sum(self, r1, r2, num):
        for i in range(self.size[1]):
    ...
    def row_echelon(self):
        ...
        for i in range(result.size[0]):
            result.div(i, result.get_row(i)[0])

        for j in range(0, i):
            result.mul(i, -1)
            result.sum(i, j)
            result.div(i, result.get_row(i)[j+1])
    ...
    def reduced_row_echelon(self):
        ...
        for i in range(result.size[0]-2, -1, -1):
            for j in range(i+1, result.size[0]):
                result.mul_then_sum(i, j, -result.get_row(i)[j])
